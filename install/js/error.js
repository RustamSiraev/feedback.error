function ALXErrorSendClass()
{
        ...
        function getSelectedText(){
                ...
                if(_select.length>250){
                        ...
                        var content_LongText='\
                                <div id="feedback_SendError_LongText">\
                                                <div style="color: red; font-size: 20px"><b>'+errSendMsg.LongText+'</b></div><br /><br />'+errSendMsg.LongText2+'\
                                </div>';

        BX.AlxErrDialog_DLongTxt = new BX.PopupWindow("feedback_ErrorWindow", window.body, {
						...
						setTimeout(() => BX.AlxErrDialog_DLongTxt.close(), 3000)
                        return false;
                }
        ...
        function esErrorSend(){
                ...
                BX.ajax.post(url,oData,function(res){
                        ...
                        if(trim(res)!="OK!")
                                document.getElementById('popup-window-content-feedback_ErrorWindow').innerHTML = content_no.replace("#ERROR#",res);
                        else
                                document.getElementById('popup-window-content-feedback_ErrorWindow').innerHTML = content_ok.replace("#ERROR#",content_ok);
                        BX.AlxErrDialog_ok.show();
						setTimeout(() => BX.AlxErrDialog_ok.close(), 3000)
                });
        }
        ...
        var content_no='<div id="feedback_SendErrorOk"><div style="color: red; font-size: 20px"><b>'+errSendMsg.senderror+'</b></div><span style="font-size:12px;color:#7d7d7d"><br />#ERROR#</span></div>';
        var content_ok='\
        <div id="feedback_SendErrorOk">\
...
BX.AlxErrDialog_ok = new BX.PopupWindow("feedback_ErrorWindow", window.body, {
                 content: content_ok,
                 closeIcon: {right: "20px", top: "10px"},
                 titleBar: "",
                 zIndex: 0,
...
<div id="send-error">\
        ...
        <div style="width:470px;max-width: 100%;min-height:55px;padding:8px;border:1px solid #4b4b4b;margin:3px 0px 3px 0px;box-sizing:content-box;"><textarea name="comment" id="error-comment" rows=3 cols=5 style="width:100%;border:0px;min-height:55px;padding:0px;min-height:55px;"></textarea></div>\
         ...
         BX.AlxErrDialog = new BX.PopupWindow("feedback_ErrorWindow", window.body, {
                 content: contentErr,
                 closeIcon: {right: "20px", top: "10px"},
                 titleBar: {content: BX.create("div", {html: errSendMsg.footer, 'props': {'className': 'feedback_ErrorTitleBar'}})},
                 //'<div class="feedback_ErrorTitleBar"><span style="font-size:16px;color:#143f6d">'+errSendMsg.footer+'</span></div>',
